...
type btnode = Leaf | Node of int * btnode * btnode
...
let rec inorder (root : btnode) : int list =
  match root with
  | Leaf -> []
  | Node (num, left, right) -> inorder left @ [num] @ inorder right

let rec preorder (root : btnode) : int list =
  match root with
  | Leaf -> []
  | Node (num, left, right) -> [num] @ preorder left @ preorder right

let rec postorder (root : btnode) : int list =
  match root with
  | Leaf -> []
  | Node (num, left, right) -> postorder left @ postorder right @ [num]

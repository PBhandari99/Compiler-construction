...
let int_list_to_string (l : int list) : string =
    let rec to_string (l : int list) : string =
        ...
        | [] -> ""
        | first::rest ->
                (string_of_int first) ^ "; " ^ (to_string rest) in
    "[ " ^ (to_string l) ^ "]";;
...
let str_list_to_string (l : string list) : string =
    let rec to_string (l : string list) : string =
        ...
        | [] -> ""
        | first::rest ->
                first ^ "; " ^ (to_string rest) in
    "[ " ^ (to_string l) ^ "]";;


let t_list name value expected conv_func = name>::
    (fun _ -> assert_equal expected value ~printer:conv_func);;
...
let list_test_1 = t_list "list_test_1" (increment_all [1;2;3;4]) [2;3;4;5] int_list_to_string;;
let list_test_2 = t_list "list_test_2" (increment_all []) [] int_list_to_string;;
let long_strings_1 = t_list "long_string_1" (long_strings [] 1) [] str_list_to_string;;
let long_strings_2 = t_list "long_string_2" (long_strings ["hello"; "gold"; "farmer"; "catches"; "fish"] 5) ["farmer"; "catches"] str_list_to_string;;
let every_string_test_1 = t_list "every_string_test_1" (every_other ["hello"; "gold"; "farmer"; "catches"; "fish"]) ["hello"; "farmer"; "fish"] str_list_to_string;;
let every_string_test_2 = t_list "every_string_test_2" (every_other ["hello"]) ["hello"] str_list_to_string;;
let every_string_test_3 = t_list "every_string_test_3" (every_other []) [] str_list_to_string;;
let sum_all_test_1 = "sum_all_test_1">::(fun _ -> assert_equal (sum_all [[1;2;3;3]; [3;2;53;54]]) [9;112]);;
let sum_all_test_2 = "sum_all_test_2">::(fun _ -> assert_equal (sum_all []) []);;
let sum_all_test_3 = "sum_all_test_3">::(fun _ -> assert_equal (sum_all [[]]) [0]);;
